...
recs_last_mod <- recs_last %>% mutate(last_rec = case_when(
  hour(last_rec) == 18 & month(last_rec) == 8 & mday(last_rec) == 15 ~ as.POSIXct("2023-08-16 00:00:00")))
# add a new column for the last date in the recording period
...
#date_deployed <- deploy %>% select(point_id, date_deployed)
...
# test <- prob_period %>% mutate(Problem1_from = case_when(
#   hour(Problem1_from) == 1 & minute(Problem1_from) == 0 & second(Problem1_from) == 0 ~ as.POSIXct(paste0(format(Problem1_from, "%Y-%m-%d"), " 00:00:00")),
#   hour(Problem1_from) == 7 & minute(Problem1_from) == 0 & second(Problem1_from) == 0 ~ as.POSIXct(paste0(format(Problem1_from, "%Y-%m-%d"), " 06:00:00")),
#   hour(Problem1_from) == 9 & minute(Problem1_from) == 0 & second(Problem1_from) == 0 ~ as.POSIXct(paste0(format(Problem1_from, "%Y-%m-%d"), " 12:00:00")),
#   hour(Problem1_from) == 23 & minute(Problem1_from) == 0 & second(Problem1_from) == 0 ~ as.POSIXct(paste0(format(Problem1_from, "%Y-%m-%d"), " 18:00:00")))) 
