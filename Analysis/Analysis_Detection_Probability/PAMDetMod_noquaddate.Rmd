...
This is the fifth version of the model I will use for the ARU detection probability. I will be incorporating a metric for detector type at each site and removing the quadratic effect on date to compare WAIC
...
$$
p_{ij} = \beta_0 + \beta_{1}*date + \beta_{2}^2 * date + \beta_{3} * avg db + \beta_{4}*effort + \beta_{5}*vegdense + \beta_{6}*aru model
$$
...
cat("
  model{
    ...
    for(i in 1:n_unit){
      ...
      for(j in 1:miss[i]){
        ...
        p[i,j] <- b0 + b1*covb1[i,j] + b3*covb3[i,j] + b4*covb4[i,j] + b5*covb5[i] + b6*covb6[i]
        # Det NA
  ...
  ", file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Methods_Chapter/ARU_Model/Model_Structure_Files/JAGS_ARUMod5.txt")
```

```{R Run Model, eval= TRUE, results = FALSE}
fit_5 <- jags(data = jags_data, 
                 model.file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Methods_Chapter/ARU_Model/Model_Structure_Files/JAGS_ARUMod5.txt",
                 parameters.to.save = c("a0", "b0", "b1", "b3", "b4", "b5","b6", "SSEsim","SSEobs","p_val"),
                 n.iter = 10000, 
                 n.burnin = 5000, 
                 n.chains = 3,
                 n.thin = 2,
...
saveRDS(fit_5, "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Methods_Chapter/ARU_Model/Models_Ran_Outputs/MetChap_ARUMod5.Rdata")
```

### Model outputs: 
```{R, eval = FALSE, echo = FALSE}
fit_5 <- readRDS("C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Methods_Chapter/ARU_Model/Models_Ran_Outputs/MetChap_ARUMod5.Rdata")
```
```{R}
summary(fit_5)
```
...
```{R, echo = FALSE}
tracedens_jags(fit_5, parmfrow = c(3,4))
```

```{R Make Inference on P Value, echo = FALSE}
plot(fit_5$sims.list$SSEobs,fit_5$sims.list$SSEsim,xlab="SSEobs",ylab="SSEsim")
abline(0,1,lwd = 2, col = "red")
print(paste0("P value is:",round(fit_5$mean$p_val, 2)))
```
...
chains_m <- jags_df(fit_5)
# Select the chains for our covariates of interest
chains_viol <- chains_m %>% select(b1,b3,b4,b5,b6)
...
colnames(chains_viol) <- c("date","background_noise","effort", "veg_density", "SMM present")
# Pivot this longer so that we can visualize it
chains_viol_long <- chains_viol %>% pivot_longer(cols = c("date","background_noise","effort", "veg_density", "SMM present"),names_to = "parameter", values_to = "values")
...
f_stat <- data.frame(
  parameter = c("date",
                ...
                "veg_density",
                "SMM present"),
  median_value = c(paste0("F: ", round(fit_5$f$b1,2)), 
                   paste0("F: ",round(fit_5$f$b3,2)), 
                   paste0("F: ",round(fit_5$f$b4,2)),
                   paste0("F: ",round(fit_5$f$b5,2)),
                   paste0("F: ",round(fit_5$f$b6,2)))
)
...
```{R Double Observer Method, eval = FALSE, echo = FALSE}
